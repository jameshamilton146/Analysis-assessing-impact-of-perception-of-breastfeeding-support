...
for(i in seq_along(modeling_data_list)){
  ...
  temp_controlOrdModel <- polr(control_formula, data = temp_dt)
  
  temp_stepwiseOrdModel <- stepAIC(temp_controlOrdModel)
  optimal_control_variable_subset <- temp_stepwiseModel$xlevels %>% names()
  optimal_control_variable_subset_ord <- temp_stepwiseOrdModel$xlevels %>% names()
full_formula <- as.formula(paste("breast_feeding_intensity ~ ", 
                         paste(c(response_var, optimal_control_variable_subset), collapse = "+")))
  
full_Ordformula <- as.formula(paste("breast_feeding_intensity ~ ", 
                         paste(c(response_var, optimal_control_variable_subset_ord), collapse = "+")))
  temp_fullModel <- multinom(full_formula, 
            data = temp_dt)
  
  temp_ordModel <- polr(full_Ordformula, data = temp_dt)
  if(i == 1){
...
coef_dt[, variable := NULL]
  model_fit[[i]] <- list(full_model = temp_fullModel,
       ...
       optimal_ord_variables = optimal_control_variable_subset_ord,
       ord_likelihood_ratio_test = lrtest(temp_ordModel, temp_stepwiseOrdModel))
}
...
# 
#   drive_rm(paste0(names(coef_list)[i], ".csv"))
...
test <- model_fit[["1_month"]][["full_model"]]

test2 <- logitgof(modeling_data_list[[1]]$breast_feeding_intensity, fitted(test))
```
...
test <- data_exploration_dt[breast_feeding_difficulties == TRUE & breast_feeding_support == TRUE & !is.na(perception_of_support),
                            .N, keyby = .(BFHI_score)]
break_options <- c(0, .2, .8, 1)
...
cat_control_dt <- lapply(control_vars, function(x){
  temp_dt <- data_exploration_dt[breast_feeding_difficulties == TRUE, .N, keyby = c(x, "perception_of_support")]
  setnames(temp_dt, x, "control_var_level")
  temp_dt[, control_var := x]
...
# fwrite(cat_control_dt, "categorizing_num_of_resp_into_each_cat.csv")
# 
...
model_fit[["1_month"]][["ord_likelihood_ratio_test"]]
```
